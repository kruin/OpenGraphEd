...
public class KruinShortestPathOperation
{
  ...
  public static Graph getShortestPath(Graph g, Node source, Node dest, boolean copyData)
  {
    ...
    KruinNodeEx sourceNode, destNode, currentNode, otherNode;
    KruinEdgeEx currentEdge, dummyEdge = new KruinEdgeEx();
    Vector incidentEdges, kruinEdges = new Vector();

    //g.clearNodeLabels();
    //g.removeEdgeDirections();

    Vector nodes = g.createNodeExtenders(new KruinNodeEx().getClass());
    g.createEdgeExtenders(new KruinEdgeEx().getClass());

    for ( int i=0; i<nodes.size(); i++ )
    {
      currentNode = (KruinNodeEx)nodes.elementAt(i);
      currentNode.setCost(Double.MAX_VALUE);
      currentNode.setTraceBackEdge(null);
      currentNode.setIsDone(false);
    }

    sourceNode = (KruinNodeEx)source.getExtender();
    destNode = (KruinNodeEx)dest.getExtender();
    ...
    do
    {
      currentNode = (KruinNodeEx)heap.extractMin();
      ...
      {
        traceBack(destNode, dummyEdge, kruinEdges);
        destReached = true;
        break;
      }
      ...
      {
        currentEdge = (KruinEdgeEx)incidentEdges.elementAt(i);
        length = currentEdge.getLength();
        otherNode = (KruinNodeEx)currentEdge.otherEndFrom(currentNode);
        if ( !otherNode.isDone() &&
    ...
    if ( destReached )
    {
      ...
      g.stopLogEntry(logEntry);
      return g.copyEdges(kruinEdges, copyData);
    }
    else
    {
      logEntry.setData("No Path Found");
  ...
  private static void traceBack(KruinNodeEx node, KruinEdgeEx dummyEdge, Vector kruinEdges)
  {
    if ( node.getTraceBackEdge() != null )
    {
      if ( node.getTraceBackEdge() != dummyEdge )
      {
        kruinEdges.addElement(node.getTraceBackEdge().getRef());
        traceBack( (KruinNodeEx)node.getTraceBackEdge().otherEndFrom(node), dummyEdge, kruinEdges );
      }
    }
  }
